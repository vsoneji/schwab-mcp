...
function formatResponse(response) {
    ...
    if ('ok' in response) {
        if (isOk(response)) {
            ...
            var message = ('message' in response && response.message) ||
            ...
            if ('details' in response && response.details) {
                ...
                if (Object.values(diagnosticInfo).some(function (val) { return val !== undefined; })) {
                    content.push({
                        type: 'text',
...
function isSchwabApiError(error) {
    ...
    return (error &&
        ...
        (error.name === 'SchwabApiError' ||
            ((_a = error.constructor) === null || _a === void 0 ? void 0 : _a.name) === 'SchwabApiError'));
}
function isAuthError(error) {
    ...
    return (error &&
        ...
        (error.name === 'SchwabAuthError' ||
            ((_a = error.constructor) === null || _a === void 0 ? void 0 : _a.name) === 'SchwabAuthError'));
}
function toolError(message, details) {
    ...
    if (isSchwabApiError(error) || isAuthError(error)) {
        var apiError = error;
        enhancedDetails = __assign(__assign({}, enhancedDetails), { status: apiError.status, code: apiError.code, parsedError: apiError.parsedError });
...
function toolSuccess(_a) {
    var data = _a.data, message = _a.message, source = _a.source;
    var count = Array.isArray(data) ? data.length : 1;
    log_1.logger.debug("Tool success: ".concat(source), {
        dataPreview: Array.isArray(data) ? "Array of ".concat(count, " items") : typeof data,
        count: count,
    });
...
function createTool(client, server, _a) {
    var _this = this;
    var name = _a.name, description = _a.description, schema = _a.schema, handler = _a.handler;
    // Populate the internal toolRegistry
    toolRegistry.set(name, { schema: schema, handler: handler });
    log_1.logger.info("[ToolBuilder] Added tool '".concat(name, "' to internal toolRegistry."));
    // Keep individual tool registration with McpServer for potential direct calls
    // or if the dispatcher logic is ever removed.
    log_1.logger.info("[ToolBuilder] Registering tool with McpServer for direct call: '".concat(name, "'."));
    server.tool(name, description, schema instanceof zod_1.z.ZodObject ? schema.shape : {}, function (args) { return __awaiter(_this, void 0, void 0, function () {
        var parsedInput, result, error_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 2, , 3]);
                    log_1.logger.info("[ToolBuilder] Direct invocation of tool: ".concat(name));
                    parsedInput = void 0;
                    try {
                        parsedInput = schema.parse(args);
                    }
                    catch (validationError) {
                        log_1.logger.error("Input validation error in direct tool: ".concat(name), {
                            validationError: validationError instanceof Error
                                ? validationError.message
                                : String(validationError),
                            argsReceived: args,
                        });
                        return [2 /*return*/, formatResponse(toolError('Invalid input for direct tool call.', {
                ...
                case 2:
                    error_1 = _a.sent();
                    log_1.logger.error("Unexpected error in direct tool: ".concat(name), {
                        error: error_1 instanceof Error ? error_1.message : String(error_1),
                    });
                    return [2 /*return*/, formatResponse(toolError(error_1 instanceof Error
                            ? error_1
